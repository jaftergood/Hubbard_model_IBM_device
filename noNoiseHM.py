...
if __name__ == "__main__":
    ...
    def M_1(m, n, thetas, num_layers, num_sites, num_circuits, back, sv_bool=True):
        ...
        if res.result().get_counts(0).get('1') is None:
            num = 0
        else:
            num = res.result().get_counts(0)['1']
        p1 = num/sum(res.result().get_counts(0).values())
        M_1_res = 1 - 2.*p1
        return M_1_res
    ...
    def vee(n):
        ...
        if res_sq.result().get_counts(0).get('1') is None:
            num = 0
        else:
            num = res_sq.result().get_counts(0)['1']
        p2 = num/sum(res_sq.result().get_counts(0).values())
            ...
            for ham_type in ['I', 'X', 'Z']:
                for ham_site in range(Nl):
                    ...
                    if res.result().get_counts(0).get('1') is None:
                        ...
                        num = res.result().get_counts(0)['1']
                    p1 = num/sum(res_sq.result().get_counts(0).values())
    ...
    def doc():
        ...
        if res_sz.result().get_counts(0).get('1') is None:
            num = 0
        else:
            num = res_sz.result().get_counts(0)['1']
        p2 = num/sum(res_sz.result().get_counts(0).values())
    ...
    if (os.path.exists(f'./noNoise/res_{dt}_{shots}_{u}_{tf}/')):
        with open(f'./noNoise/res_{dt}_{shots}_{u}_{tf}/thetas_{dt}_{shots}_{u}_{tf}.pkl','wb') as f:
            pickle.dump(delt_thet,f)

        with open(f'./noNoise/res_{dt}_{shots}_{u}_{tf}/doub_occ_{dt}_{shots}_{u}_{tf}.pkl','wb') as f:
            pickle.dump(doub_occ,f)
    else:
        os.mkdir(f'./noNoise/res_{dt}_{shots}_{u}_{tf}')
        with open(f'./noNoise/res_{dt}_{shots}_{u}_{tf}/thetas_{dt}_{shots}_{u}_{tf}.pkl','wb') as f:
            pickle.dump(delt_thet,f)
